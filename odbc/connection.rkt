      ...
      (call-as-atomic
       (lambda ()
         (let ([stmt* stmt])
           (when stmt*
             (set! stmt #f)
             (handle-status 'finalize-statement (SQLFreeStatement stmt* SQL_CLOSE))
             (handle-status 'finalize-statement (SQLFreeHandle SQL_HANDLE_STMT stmt*))
             (void))))))
...
(define connection%
  (class* object% (connection<%>)
         ...
         (let* ([db (get-db fsym)]
                ...
                [pinned-buffers
                 (for/list ([i (in-naturals 1)]
                            ...
                            [param-type (in-list (send pst get-param-typeids))])
                   (load-param fsym db stmt i param param-type))]
                [result-infos (send pst get-result-infos)]
                [rows (fetch* fsym db stmt)])
           (handle-status fsym (SQLFreeStmt stmt SQL_RESET_PARAMS))
    ...
    (define/private (load-param fsym db stmt i param param-type)
      ;; param buffers must not move between bind and execute
      (define (make-buffer n) (malloc n 'raw))
