...
#|
marshal-<type> : racket-datum -> string (???)
...
(define (marshal-integer n)
  (unless (exact-integer? n)
    (marshal-error "integer" n))
  (number->string n))

(define (marshal-int* n type min max)
  (unless (and (exact-integer? n) (<= min n max))
    (marshal-error type n))
  (number->string n))

(define (marshal-int1 n)
  (marshal-int* n "int1" #x-80 #x7F))

(define (marshal-int2 n)
  (marshal-int* n "int2" #x-8000 #x7FFF))

(define (marshal-int3 n)
  (marshal-int* n "int3" #x-800000 #x7FFFFF))

(define (marshal-int4 n)
  (marshal-int* n "int4" #x-80000000 #x7FFFFFFF))

(define (marshal-int8 n)
  (marshal-int* n "int8" #x-8000000000000000 #x7FFFFFFFFFFFFFFF))

(define (marshal-real n)
  (unless (real? n)
    (marshal-error "real" n))
...
(define (marshal-error type datum)
  (error 'query* "cannot marshal as SQL type ~s: ~e"
         type datum))

;; make-default-marshal : Type -> datum -> string
(define ((make-default-marshal type) datum)
  (cond [(string? datum) datum]
        [else (marshal-error type datum)]))
